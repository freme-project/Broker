...
public class RateLimitingFilterTest extends EServiceTest {
    ...
    public void testRatelimiting() throws UnirestException, IOException {
        ...
        response = addAuthentication(get("/e-entity/freme-ner/datasets"), ratelimiterToken).asString();
        ...
        loggerIgnore("TooManyRequestsException");
        response = addAuthentication(post("/e-translation/tilde"), ratelimiterToken)
        ...
        response = get("/e-link/templates").asString();
        assertEquals(HttpStatus.OK.value(), response.getStatus());
    }
}
